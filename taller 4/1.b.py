...
dx=0.04; dx2=dx*dx; k_0=-2; dt=dx2/20.0; xmax=10.0
xs=np.arange(-xmax, xmax+dx/2, dx)
a=0.1

psr=np.exp(-2*(xs-0)**2)*np.cos(k_0*xs)
psi=np.exp(-2*(xs-0)**2)*np.sin(k_0*xs)
v=(1/5**4)*xs**4
...
fig2.tight_layout(); fig2.savefig('1.b.pdf')
